...
parser = argparse.ArgumentParser(description='指定した年の1分足データを処理します')
...
logger.log_info(f"{year}年の1分足データ処理開始")
...
os.makedirs(f"data/processed/1min/{year}", exist_ok=True)
...
logger.log_info(f"1分足データを処理中...")
try:
    data_with_indicators = data_processor.add_technical_indicators(data)
    
    file_path = f"data/processed/1min/{year}/USDJPY_1min_{year}.csv"
    data_with_indicators.to_csv(file_path)
    logger.log_info(f"{file_path} に保存完了: {len(data_with_indicators)} 行")
except Exception as e:
    logger.log_error(f"1分足データの処理中にエラーが発生しました: {e}")

try:
    resampled_15min = data_processor.resample('15min')
    if not resampled_15min.empty:
        resampled_15min = data_processor.add_technical_indicators(resampled_15min)
        
        file_path = f"data/processed/15min/{year}/USDJPY_15min_{year}.csv"
        resampled_15min.to_csv(file_path)
        logger.log_info(f"{file_path} に保存完了: {len(resampled_15min)} 行")
    else:
        logger.log_warning(f"15分足へのリサンプリング結果が空です")
except Exception as e:
    logger.log_error(f"15分足へのリサンプリング中にエラーが発生しました: {e}")
