...
logger.log_info("長期戦略のテスト開始（2020-2024年）")

results_df = pd.DataFrame(columns=['年', 'トレード数', '勝率', 'プロフィットファクター', '純利益', '年利'])

test_years = [2020, 2021, 2022, 2023, 2024]

for test_year in test_years:
    logger.log_info(f"=== {test_year}年のテスト開始 ===")
    ...
    try:
        ...
        if not trades.empty:
            ...
            annual_return = (net_profit / 2000000) * 100
            ...
            results_df = pd.concat([results_df, pd.DataFrame([{
                ...
                '純利益': f"{net_profit:.2f}円",
                '年利': f"{annual_return:.2f}%"
            }])], ignore_index=True)
    ...
    logger.log_info(f"=== {test_year}年のテスト完了 ===")

if not results_df.empty:
    report_path = "document/strategy_optimization/long_term_strategy_results_2020_2024.md"
    os.makedirs("document/strategy_optimization", exist_ok=True)
    
    with open(report_path, "w") as f:
        f.write(f"# 長期戦略テスト結果 (2020-2024)\n\n")
        f.write(f"## テスト環境\n")
        f.write(f"- 戦略: マクロ経済要因に基づく長期戦略 (MacroBasedLongTermStrategy)\n")
        f.write(f"- 時間足: 4H, 1D, 1W, 1M\n")
        ...
        for _, row in results_df.iterrows():
            f.write(f"| {row['年']} | {row['トレード数']} | {row['勝率']} | {row['プロフィットファクター']} | {row['純利益']} | {row['年利']} |\n")
        
        avg_trades = results_df['トレード数'].mean()
        avg_win_rate = np.mean([float(rate.strip('%')) for rate in results_df['勝率']])
        avg_pf = np.mean([float(pf) for pf in results_df['プロフィットファクター']])
        total_profit = sum([float(profit.strip('円')) for profit in results_df['純利益']])
        avg_annual_return = np.mean([float(rate.strip('%')) for rate in results_df['年利']])
        
        f.write(f"| **平均/合計** | **{avg_trades:.0f}** | **{avg_win_rate:.2f}%** | **{avg_pf:.2f}** | **{total_profit:.2f}円** | **{avg_annual_return:.2f}%** |\n\n")
        
        f.write(f"## 分析\n\n")
        f.write(f"長期戦略の結果分析：\n")
        f.write(f"- 平均取引数: {avg_trades:.0f}\n")
        f.write(f"- 平均勝率: {avg_win_rate:.2f}%\n")
        f.write(f"- 平均プロフィットファクター: {avg_pf:.2f}\n")
        f.write(f"- 総純利益: {total_profit:.2f}円\n")
        f.write(f"- 平均年利: {avg_annual_return:.2f}%\n\n")
        ...
        f.write(f"- 現在の平均勝率: {avg_win_rate:.2f}%\n")
        f.write(f"- 損益分岐点からの差: {avg_win_rate - 25:.2f}%\n\n")
    ...
    logger.log_warning("テスト結果がありません")

logger.log_info("長期戦略のテスト完了（2020-2024年）")
