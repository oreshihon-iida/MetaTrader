...
log_dir = "results/final_optimization/logs"
chart_dir = "results/final_optimization/charts"
os.makedirs(log_dir, exist_ok=True)
os.makedirs(chart_dir, exist_ok=True)

logger = Logger(log_dir)
logger.log_info("最終最適化戦略のテスト開始")
...
strategy = BollingerRsiEnhancedMTStrategy(
    ...
    rsi_upper=85,                  # RSI上限をさらに厳格化（80→85）
    rsi_lower=15,                  # RSI下限をさらに厳格化（20→15）
    bb_dev=2.2,                    # ボリンジャーバンド偏差を増加（2.0→2.2）
    sl_pips=15.0,                  # ストップロスを維持
    tp_pips=45.0,                  # テイクプロフィットを広げる（30.0→45.0）
    consecutive_limit=1,           # 連続シグナル制限を維持
...
if not results.empty:
    ...
    with open(f"{log_dir}/backtest_summary.md", "w") as f:
        ...
        f.write("- RSI上限: 85\n")
        f.write("- RSI下限: 15\n")
        f.write("- ボリンジャーバンド偏差: 2.2\n")
        f.write("- ストップロス: 15.0 pips\n")
        f.write("- テイクプロフィット: 45.0 pips\n")
        f.write("- 連続シグナル制限: 1\n")
        f.write("- 季節性フィルター: 有効\n")
