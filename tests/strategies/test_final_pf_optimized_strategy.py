...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/test_final_pf_optimized_strategy.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

output_dir = 'results/test_final_pf_optimized_strategy'
...
logger.info(f"最終PF最適化版短期戦略のテスト開始")
...
try:
    ...
    strategy = FinalPFOptimizedStrategy()
    ...
    if trades > 0:
        ...
        visualizer.plot_equity_curve(equity_curve, f'final_pf_optimized_strategy_equity_curve_{test_year}')
        ...
        visualizer.plot_drawdown(equity_curve, f'final_pf_optimized_strategy_drawdown_{test_year}')
        ...
        visualizer.plot_monthly_returns(months, profits, f'final_pf_optimized_strategy_monthly_returns_{test_year}')
        
        with open(f'{output_dir}/reports/final_pf_optimized_strategy_results.md', 'w') as f:
            f.write(f"# 最終PF最適化版短期戦略のバックテスト結果\n\n")
            f.write(f"## 概要\n\n")
            f.write(f"プロフィットファクター1.5を目標とした最終PF最適化版短期ボリンジャーバンド＋RSI戦略のバックテスト結果です。\n\n")
            f.write(f"## 総合結果\n\n")
            f.write(f"| 項目 | 値 |\n")
            f.write(f"| --- | --- |\n")
            ...
            f.write(f"## パラメータ設定\n\n")
            f.write(f"| パラメータ | 改良版短期戦略 | バランス最適化版 | 最終PF最適化版 | 説明 |\n")
            f.write(f"| --- | --- | --- | --- | --- |\n")
            f.write(f"| bb_dev | 1.6 | 1.7 | 1.7 | ボリンジャーバンド幅を適度に広げる |\n")
            f.write(f"| rsi_upper | 55 | 65 | 70 | RSI閾値を調整（非常に強い過買い状態でのみ売り） |\n")
            f.write(f"| rsi_lower | 45 | 35 | 30 | RSI閾値を調整（非常に強い過売り状態でのみ買い） |\n")
            f.write(f"| sl_pips | 3.0 | 3.0 | 2.5 | 損切り幅を縮小 |\n")
            f.write(f"| tp_pips | 7.5 | 10.5 | 12.5 | 利確幅を大幅に拡大してリスク・リワード比を1:5に改善 |\n")
            f.write(f"| trend_filter | False | True | True | トレンドフィルターを有効化 |\n")
            f.write(f"| vol_filter | True | True | True | ボラティリティフィルターを維持 |\n")
            f.write(f"| time_filter | True | True | True | 時間フィルターを維持 |\n")
            f.write(f"| ATR閾値 | 0.8 | 0.8 | 0.85 | ATR閾値を引き上げ |\n")
            f.write(f"| 価格アクション確認 | なし | なし | 3本連続の同方向の動き | 価格アクションの確認を追加 |\n\n")
            
            f.write(f"## 主な改善点\n\n")
            f.write(f"1. **RSI閾値の最適化**: 買いシグナルは30、売りシグナルは70に調整（非常に強い過買い/過売り状態でのみ取引）\n")
            f.write(f"2. **リスク・リワード比の大幅改善**: 1:2.5から1:5に拡大（SL:2.5, TP:12.5）\n")
            f.write(f"3. **トレンドフィルターの強化**: 短期・中期・長期の移動平均線がトレンド方向に揃っている場合のみ取引\n")
            f.write(f"4. **ボリンジャーバンドの乖離率チェックの厳格化**: バンドからの乖離が非常に大きい場合のみ取引\n")
            f.write(f"5. **価格アクションの確認**: 3本連続の同方向の動きを確認して反転の可能性を高める\n")
            f.write(f"6. **ポジションサイズ管理の強化**: 連続損失時は40%まで削減し、連続勝利時は最大1.8倍まで増加\n\n")
            
            f.write(f"## 結論\n\n")
            
            if profit_factor >= 1.5:
                f.write(f"最終PF最適化版短期戦略は中間目標のプロフィットファクター1.5以上を達成しました。\n")
                f.write(f"主な成功要因は以下の通りです：\n\n")
                f.write(f"1. リスク・リワード比の大幅な改善（1:5）\n")
                f.write(f"2. RSI閾値の最適化（30/70）\n")
                f.write(f"3. トレンドフィルターの強化\n")
                f.write(f"4. 価格アクションの確認の導入\n\n")
                f.write(f"次のステップとして、勝率の向上（目標：70%以上）とプロフィットファクターのさらなる向上（目標：2.0以上）を目指すことをお勧めします。\n")
            else:
                f.write(f"最終PF最適化版短期戦略は中間目標のプロフィットファクター1.5以上を達成できませんでした。\n")
                f.write(f"さらなる改善のために以下の点を検討してください：\n\n")
                f.write(f"1. RSI閾値のさらなる最適化\n")
                f.write(f"2. リスク・リワード比のさらなる改善\n")
                f.write(f"3. 市場環境に応じた動的パラメータ調整の実装\n")
                f.write(f"4. 複合指標の開発（RSI+ボリンジャーバンド+MACD等）\n")
        
        logger.info(f"テスト完了: 結果は {output_dir}/reports/final_pf_optimized_strategy_results.md に保存されました")
    else:
