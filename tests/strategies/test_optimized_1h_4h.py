...
log_dir = "results/optimized_1h_4h/logs"
chart_dir = "results/optimized_1h_4h/charts"
os.makedirs(log_dir, exist_ok=True)
os.makedirs(chart_dir, exist_ok=True)

logger = Logger(log_dir)
logger.log_info("最適化された1時間足+4時間足戦略のテスト開始")
...
strategy = BollingerRsiEnhancedMTStrategy(
    ...
    rsi_upper=80,              # RSI上限を厳格化（75→80）
    rsi_lower=20,              # RSI下限を厳格化（25→20）
    bb_dev=2.0,                # ボリンジャーバンド偏差を元に戻す（1.8→2.0）
    sl_pips=15.0,              # ストップロスを広げる（10.0→15.0）
    tp_pips=30.0,              # テイクプロフィットを広げる（15.0→30.0）
    consecutive_limit=1,       # 連続シグナル制限を厳格化（3→1）
...
if not results.empty:
    ...
    with open(f"{log_dir}/backtest_summary.md", "w") as f:
        f.write("# 最適化された1時間足+4時間足戦略のバックテスト結果\n\n")
        ...
        if win_rate >= 70 and profit_factor >= 2.0:
            ...
            if win_rate < 70:
                f.write("- 勝率が目標（70%）を下回っています。シグナル生成条件をさらに厳格化することを検討してください。\n")
            
            if profit_factor < 2.0:
                f.write("- プロフィットファクターが目標（2.0）を下回っています。損切り・利確レベルの調整を検討してください。\n")
        
        f.write("\n## 今後の改善点\n\n")
        f.write("1. **シグナル生成条件のさらなる最適化**：\n")
        f.write("   - RSIの条件をさらに厳格化\n")
        f.write("   - ボリンジャーバンドの条件を調整\n")
        f.write("   - 時間足の重み付けを見直し\n\n")
        
        f.write("2. **フィルターの強化**：\n")
        f.write("   - 時間帯フィルターの精緻化\n")
        f.write("   - 曜日フィルターの調整\n")
        f.write("   - 経済指標発表前後の取引制限\n\n")
        
        f.write("3. **損切り・利確レベルの最適化**：\n")
        f.write("   - ATRに基づく動的な損切り・利確レベルの調整\n")
        f.write("   - リスク・リワード比の見直し\n")
    
    logger.log_info(f"バックテスト結果のサマリーを保存しました: {log_dir}/backtest_summary.md")
else:
    logger.log_warning("取引履歴がありません")

logger.log_info("最適化された1時間足+4時間足戦略のテスト完了")
