...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/test_enhanced_dual_strategy.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

output_dir = 'results/test_enhanced_dual_strategy'
...
logger.info(f"拡張デュアル戦略（同時ポジション数5、勝率条件付きロットサイズ調整）のテスト開始 - {test_year}年")
...
try:
    ...
    strategy_manager = EnhancedDualStrategyManager(
        short_term_strategy_params=short_term_params,
        long_term_strategy_params=long_term_params,
        max_short_term_positions=3,  # 短期戦略の最大ポジション数を3に設定
        max_long_term_positions=2,   # 長期戦略の最大ポジション数を2に設定
        short_term_capital_ratio=0.4,  # 短期戦略の資金配分比率を0.4に調整
        long_term_capital_ratio=0.6    # 長期戦略の資金配分比率を0.6に調整
    )
    ...
    if short_term_signal_count > 0:
        ...
        short_term_backtest = EnhancedBacktestEngine(
            short_term_signals, 
            initial_balance=1000000,  # 初期資金を100万円に設定
            base_lot_size=0.01,       # 基本ロットサイズを0.01に設定
            max_positions=3,          # 短期戦略の最大ポジション数を3に設定
            spread_pips=0.03,
            win_rate_threshold=80.0,  # 勝率80%以上でロットサイズを増加
            increased_lot_size=0.02   # 勝率条件達成時のロットサイズを0.02に設定
        )
    ...
    if long_term_signal_count > 0:
        ...
        long_term_backtest = EnhancedBacktestEngine(
            long_term_signals, 
            initial_balance=1000000,  # 初期資金を100万円に設定
            base_lot_size=0.01,       # 基本ロットサイズを0.01に設定
            max_positions=2,          # 長期戦略の最大ポジション数を2に設定
            spread_pips=0.03,
            win_rate_threshold=80.0,  # 勝率80%以上でロットサイズを増加
            increased_lot_size=0.02   # 勝率条件達成時のロットサイズを0.02に設定
        )
    ...
    short_term_position_limit_reached = short_term_results['position_limit_reached_count'] if short_term_results else 0
    ...
    long_term_position_limit_reached = long_term_results['position_limit_reached_count'] if long_term_results else 0
    ...
    total_position_limit_reached = short_term_position_limit_reached + long_term_position_limit_reached
    
    total_win_rate = (total_wins / total_trades * 100) if total_trades > 0 else 0
    
    logger.info(f"{test_year}年の拡張デュアル戦略バックテスト結果:")
    ...
    logger.info(f"ポジション制限到達回数: 短期={short_term_position_limit_reached}, 長期={long_term_position_limit_reached}, 合計={total_position_limit_reached}")
    ...
    if total_trades > 0:
        ...
        if short_term_results and long_term_results:
            ...
            if len(common_dates) > 0:
                ...
                visualizer.plot_equity_curves(combined_equity, f'拡張デュアル戦略_エクイティカーブ_{test_year}')
            else:
                logger.warning("短期戦略と長期戦略のエクイティカーブに共通する日付がありません")
    
    with open(f'{output_dir}/reports/enhanced_dual_strategy_results.md', 'w') as f:
        f.write(f"# 拡張デュアル戦略のバックテスト結果 - {test_year}年\n\n")
        f.write(f"## 概要\n\n")
        f.write(f"同時ポジション数5、勝率条件付きロットサイズ調整機能を持つ拡張デュアル戦略のバックテスト結果です。\n")
        f.write(f"短期戦略（15分足のみ）と長期戦略（1時間足+4時間足）を組み合わせています。\n\n")
        f.write(f"## 結果\n\n")
        ...
        f.write(f"| ポジション制限到達回数 | {short_term_position_limit_reached} | {long_term_position_limit_reached} | {total_position_limit_reached} |\n")
        ...
        f.write(f"| --- | --- | --- |\n")
        f.write(f"| 初期資金 | 1,000,000円 | 資金を100万円に設定 |\n")
        f.write(f"| 基本ロットサイズ | 0.01 | 基本のロットサイズ |\n")
        f.write(f"| 勝率条件付きロットサイズ | 0.02 | 勝率80%以上の場合のロットサイズ |\n")
        f.write(f"| 最大同時ポジション数 | 5 | 短期:3、長期:2に配分 |\n\n")
        ...
        f.write(f"| 資金配分 | 40% | 総資金の40%を短期戦略に配分 |\n")
        f.write(f"| 最大ポジション数 | 3 | 短期戦略の最大同時ポジション数 |\n\n")
        ...
        f.write(f"| 資金配分 | 60% | 総資金の60%を長期戦略に配分 |\n")
        f.write(f"| 最大ポジション数 | 2 | 長期戦略の最大同時ポジション数 |\n\n")
        
        f.write(f"## 結論\n\n")
        
        if total_win_rate >= 70 and total_profit > 0:
            f.write(f"拡張デュアル戦略は目標の勝率70%以上を達成し、プラスの純利益を生み出しました。\n")
            f.write(f"同時ポジション数を5に増やし、勝率条件付きロットサイズ調整機能を導入したことで、取引機会と収益性が向上しました。\n")
            
            if total_win_rate >= 80:
                f.write(f"勝率が80%を超えたため、ロットサイズが0.01から0.02に増加し、収益性がさらに向上しました。\n")
        else:
            f.write(f"拡張デュアル戦略は目標の勝率70%以上を達成できませんでした。\n")
            f.write(f"以下の点を検討してください：\n\n")
            f.write(f"1. 短期戦略と長期戦略のパラメータの最適化\n")
            f.write(f"2. 資金配分比率のさらなる調整\n")
            f.write(f"3. 各戦略の最大ポジション数の最適化\n")
            f.write(f"4. 勝率条件の閾値調整（現在は80%）\n")
            
        f.write(f"\n## ポジション制限の分析\n\n")
        if total_position_limit_reached > 0:
            position_limit_ratio = (total_position_limit_reached / len(df_15min)) * 100
            f.write(f"ポジション制限に達した回数は合計{total_position_limit_reached}回で、全期間の{position_limit_ratio:.2f}%を占めています。\n")
            if position_limit_ratio > 10:
                f.write(f"ポジション制限に頻繁に達しているため、最大ポジション数をさらに増やすことを検討してもよいでしょう。\n")
            else:
                f.write(f"ポジション制限に達する頻度は低く、現在の最大ポジション数5は適切と考えられます。\n")
        else:
            f.write(f"ポジション制限に達した回数は0回でした。現在の最大ポジション数5は十分であり、さらに増やす必要はないと考えられます。\n")
    
    logger.info(f"テスト完了: 結果は {output_dir}/reports/enhanced_dual_strategy_results.md に保存されました")
