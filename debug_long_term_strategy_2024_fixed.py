...
try:
    ...
    if 'Close' not in signals_df.columns:
        logger.log_info("Closeカラムが見つからないため、追加します")
        close_values = []
        for idx, row in signals_df.iterrows():
            if idx in data_dict['1D'].index:
                close_values.append(data_dict['1D'].loc[idx, 'Close'])
            else:
                closest_date = min(data_dict['1D'].index, key=lambda x: abs((x - idx).total_seconds()))
                close_values.append(data_dict['1D'].loc[closest_date, 'Close'])
                
        signals_df['Close'] = close_values
