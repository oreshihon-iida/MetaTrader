...
class FinalPFOptimizedStrategy(ImprovedShortTermStrategy):
    """
    最終プロフィットファクター最適化版短期ボリンジャーバンド＋RSI戦略
    
    プロフィットファクター1.5を目標とした最終最適化版短期戦略
    ImprovedShortTermStrategyをベースに、リスク・リワード比を大幅に改善し、
    厳選されたエントリー条件を適用した戦略
    """
    ...
    def __init__(self, **kwargs):
        ...
        **kwargs
        ...
        default_params = {
            ...
            'rsi_window': 14,
            'rsi_upper': 70,        # RSI閾値を調整（55→70）- 非常に強い過買い状態でのみ売り
            'rsi_lower': 30,        # RSI閾値を調整（45→30）- 非常に強い過売り状態でのみ買い
            'sl_pips': 2.5,         # 損切り幅を縮小（3.0→2.5）
            'tp_pips': 12.5,        # 利確幅を大幅に拡大（7.5→12.5）してリスク・リワード比を1:5に改善
            'atr_window': 14,
            'atr_sl_multiplier': 0.7,  # ATRベースの損切り乗数を縮小（0.8→0.7）
            'atr_tp_multiplier': 3.5,  # ATRベースの利確乗数を大幅に拡大（2.0→3.5）
        ...
        self.name = "最終PF最適化版短期ボリンジャーバンド＋RSI戦略"
        
        self.consecutive_losses = 0
        self.consecutive_wins = 0
        self.max_consecutive_losses = 3
        
        self.high_win_rate_hours = [0, 1, 8, 9, 13, 14]
        
    def _apply_filters(self, df: pd.DataFrame, i: int) -> bool:
        """
        各種フィルターを適用し、シグナルを生成するかどうかを判断する
        
        最終PF最適化版では非常に厳選されたエントリー条件を適用
        ...
        if self.vol_filter:
            ...
            atr = df['atr'].iloc[i]
            atr_threshold = df['atr'].rolling(window=20).mean().iloc[i] * 0.85
        ...
        if 'bb_upper' in df.columns and 'bb_lower' in df.columns:
            ...
            if df['signal'].iloc[i] > 0:
                if price > bb_lower + (bb_middle - bb_lower) * 0.1:
                    return False
                    
            if df['signal'].iloc[i] < 0:
                if price < bb_upper - (bb_upper - bb_middle) * 0.1:
                    return False
    ...
    def calculate_position_size(self, signal: int, equity: float = 10000.0) -> float:
        ...
        if self.consecutive_losses >= self.max_consecutive_losses:
            return base_size * 0.4  # 連続損失時は40%に削減
        elif self.consecutive_losses > 0:
            reduction_factor = 1.0 - (self.consecutive_losses * 0.2)
            return base_size * max(0.4, reduction_factor)
        elif self.consecutive_wins > 0:
            bonus_factor = 1.0 + (self.consecutive_wins * 0.2)
            return base_size * min(1.8, bonus_factor)
    ...
    def generate_signals(self, df: pd.DataFrame, year: int, processed_dir: str = 'data/processed') -> pd.DataFrame:
        ...
        最終PF最適化版では移動平均線を追加し、より厳格なシグナル生成条件を適用
