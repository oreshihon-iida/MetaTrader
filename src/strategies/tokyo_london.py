import pandas as pd
import numpy as np
from typing import Dict, Tuple, List, Optional

class TokyoLondonStrategy:
    """
    東京レンジ・ロンドンブレイクアウト戦略
    
    東京時間のレンジを計算し、ロンドン時間にブレイクアウトしたらエントリーする
    
    改善ポイント：
    - ADXによる市場環境フィルター（トレンド相場のみで取引）
    - 時間帯フィルター（ニューヨーク時間のボラティリティが高い時間を除外）
    - ATRベースの動的な損切り/利確幅
    - 偽ブレイクアウト対策（一定以上の値動きがあった場合のみエントリー）
    """
    
    def __init__(self, sl_pips: float = 10.0, tp_pips: float = 15.0, atr_multiplier: float = 1.5, min_adx: float = 25.0, use_higher_timeframe: bool = True):
        """
        初期化
        
        Parameters
        ----------
        sl_pips : float, default 10.0
            固定ストップロス（pips）
        tp_pips : float, default 15.0
            固定テイクプロフィット（pips）
        atr_multiplier : float, default 1.5
            ATRの乗数（動的なストップロス/テイクプロフィットの計算に使用）
        min_adx : float, default 25.0
            最小ADX値（これより低い場合はエントリーしない）
        use_higher_timeframe : bool, default True
            1時間足での確認を使用するかどうか
        """
        self.sl_pips = sl_pips
        self.tp_pips = tp_pips
        self.atr_multiplier = atr_multiplier
        self.min_adx = min_adx
        self.use_higher_timeframe = use_higher_timeframe
        self.name = "東京レンジ・ロンドンブレイクアウト"
    
    def generate_signals(self, df: pd.DataFrame, hourly_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        トレードシグナルを生成する
        
        Parameters
        ----------
        df : pd.DataFrame
            15分足のOHLCデータ
        hourly_df : pd.DataFrame, default None
            1時間足のOHLCデータ（複数時間足分析用）
               
        Returns
        -------
        pd.DataFrame
            トレードシグナルが追加されたDataFrame
        """
        df = df.copy()
        
        df['signal'] = 0
        df['entry_price'] = np.nan
        df['sl_price'] = np.nan
        df['tp_price'] = np.nan
        df['trailing_stop'] = False  # トレイリングストップフラグ
        df['strategy'] = None
        
        jst_hours = (df.index.to_series().dt.hour + 9) % 24
        
        df['valid_time'] = ~((jst_hours >= 21) & (jst_hours < 24))
        
        for i in range(1, len(df)):
            prev_row = df.iloc[i-1]
            curr_row = df.iloc[i]
            
            if not curr_row['valid_time']:
                continue
                
            if 'adx' in curr_row and curr_row['adx'] < self.min_adx:
                continue
            
            jst_hour = (df.index[i].to_pydatetime().hour + 9) % 24
            
            if jst_hour >= 16:
                tokyo_high = curr_row['tokyo_high']
                tokyo_low = curr_row['tokyo_low']
                
                higher_tf_trend = 0  # 初期値：中立
                
                if self.use_higher_timeframe and hourly_df is not None:
                    current_time = df.index[i]
                    closest_hourly_idx = hourly_df.index.get_indexer([current_time], method='pad')[0]
                    
                    if closest_hourly_idx >= 0:
                        higher_tf_row = hourly_df.iloc[closest_hourly_idx]
                        if 'trend_direction' in higher_tf_row:
                            higher_tf_trend = higher_tf_row['trend_direction']
                
                near_resistance = np.nan
                near_support = np.nan
                
                if 'nearest_resistance' in curr_row:
                    near_resistance = curr_row['nearest_resistance']
                if 'nearest_support' in curr_row:
                    near_support = curr_row['nearest_support']
                
                breakout_up = curr_row['Close'] > tokyo_high and (curr_row['Close'] - tokyo_high) > 0.5 * curr_row['atr']
                
                higher_tf_confirms = not self.use_higher_timeframe or hourly_df is None or higher_tf_trend >= 0
                
                near_key_level = False
                signal_strength = 1.0
                
                if not np.isnan(near_resistance) and curr_row['Close'] > near_resistance and (curr_row['Close'] - near_resistance) < 20 * 0.01:
                    near_key_level = True
                    if 'resistance_strength' in curr_row:
                        signal_strength = curr_row['resistance_strength']  # レジスタンスの強さに基づいてシグナル強度を設定
                
                if not np.isnan(near_support) and (curr_row['Close'] - near_support) < 30 * 0.01 and (curr_row['Close'] - near_support) > 10 * 0.01:
                    near_key_level = True
                    if 'support_strength' in curr_row:
                        signal_strength = max(signal_strength, curr_row['support_strength'])
                
                if higher_tf_trend > 0:  # 1時間足が上昇トレンド
                    signal_strength *= 1.2
                
                if breakout_up and higher_tf_confirms and (near_key_level or signal_strength > 1.0):
                    adjusted_tp = curr_row['Close'] + self.tp_pips * 0.01 * signal_strength
                    
                    if not np.isnan(near_resistance) and near_resistance > curr_row['Close'] and near_resistance < adjusted_tp:
                        adjusted_tp = near_resistance
                    
                    df.loc[df.index[i], 'signal'] = 1
                    df.loc[df.index[i], 'entry_price'] = curr_row['Close']
                    df.loc[df.index[i], 'strategy'] = self.name
                    
                    if 'atr' in curr_row:
                        sl_pips_dynamic = self.atr_multiplier * curr_row['atr'] / 0.01 / signal_strength  # 強いシグナルほど損切り幅を狭く
                        tp_pips_dynamic = self.atr_multiplier * 1.5 * curr_row['atr'] / 0.01 * signal_strength  # 強いシグナルほど利確幅を広く
                        
                        df.loc[df.index[i], 'sl_price'] = curr_row['Close'] - sl_pips_dynamic * 0.01
                        df.loc[df.index[i], 'tp_price'] = adjusted_tp
                        df.loc[df.index[i], 'trailing_stop'] = True
                    else:
                        df.loc[df.index[i], 'sl_price'] = curr_row['Close'] - self.sl_pips * 0.01 / signal_strength
                        df.loc[df.index[i], 'tp_price'] = adjusted_tp
                
                breakout_down = curr_row['Close'] < tokyo_low and (tokyo_low - curr_row['Close']) > 0.5 * curr_row['atr']
                
                higher_tf_confirms = not self.use_higher_timeframe or hourly_df is None or higher_tf_trend <= 0
                
                near_key_level = False
                signal_strength = 1.0
                
                if not np.isnan(near_support) and curr_row['Close'] < near_support and (near_support - curr_row['Close']) < 20 * 0.01:
                    near_key_level = True
                    if 'support_strength' in curr_row:
                        signal_strength = curr_row['support_strength']  # サポートの強さに基づいてシグナル強度を設定
                
                if not np.isnan(near_resistance) and (near_resistance - curr_row['Close']) < 30 * 0.01 and (near_resistance - curr_row['Close']) > 10 * 0.01:
                    near_key_level = True
                    if 'resistance_strength' in curr_row:
                        signal_strength = max(signal_strength, curr_row['resistance_strength'])
                
                if higher_tf_trend < 0:  # 1時間足が下降トレンド
                    signal_strength *= 1.2
                
                if breakout_down and higher_tf_confirms and (near_key_level or signal_strength > 1.0):
                    adjusted_tp = curr_row['Close'] - self.tp_pips * 0.01 * signal_strength
                    
                    if not np.isnan(near_support) and near_support < curr_row['Close'] and near_support > adjusted_tp:
                        adjusted_tp = near_support
                    
                    df.loc[df.index[i], 'signal'] = -1
                    df.loc[df.index[i], 'entry_price'] = curr_row['Close']
                    df.loc[df.index[i], 'strategy'] = self.name
                    
                    if 'atr' in curr_row:
                        sl_pips_dynamic = self.atr_multiplier * curr_row['atr'] / 0.01 / signal_strength  # 強いシグナルほど損切り幅を狭く
                        tp_pips_dynamic = self.atr_multiplier * 1.5 * curr_row['atr'] / 0.01 * signal_strength  # 強いシグナルほど利確幅を広く
                        
                        df.loc[df.index[i], 'sl_price'] = curr_row['Close'] + sl_pips_dynamic * 0.01
                        df.loc[df.index[i], 'tp_price'] = adjusted_tp
                        df.loc[df.index[i], 'trailing_stop'] = True
                    else:
                        df.loc[df.index[i], 'sl_price'] = curr_row['Close'] + self.sl_pips * 0.01 / signal_strength
                        df.loc[df.index[i], 'tp_price'] = adjusted_tp
        
        if 'valid_time' in df.columns:
            df = df.drop('valid_time', axis=1)
        
        return df
